...
\title{Virtual Analog Model of State Variable Filter}
\author{Jaak Jensen}
...
\begin{tikzpicture}
	...
	\matrix[row sep=2.5mm, column sep=5mm]
	{
		...
		\node[coordinate]                  (m1X3) {};          &
		\node[dspnodeopen,dsp/label=above] (m1X4) {$Y_{LP}$};    \\
		%--------------------------------------------------------------------
		\node[coordinate]		   (m20) {};          &
		\node[coordinate]                  (m21) {};          &
		\node[coordinate]                  (m22) {};          &
		\node[coordinate]                  (m23) {};          &
		\node[dspmixer,dsp/label=below]                  (m24) {$\rho$};          &
		\node[coordinate]                  (m25) {};          &
		\node[coordinate]                  (m26) {};          &
		\node[dspnodefull]                  (m27) {};          &
		\node[coordinate]                  (m28) {};          &
...
\end{tikzpicture}
\caption{Digital Model of SVF}
\end{center}
\end{figure}

\section{State Variable Filter Coefficients}
\begin{flushleft}
		\(\alpha_{0} = \frac{1}{(1+2Rg+g^2} \) \\
\(\alpha = g \) \\
\(\rho = 2R+g \) \\
\(g = w_{c}T / 2\)
\end{flushleft}
...
\begin{figure} [ht]
    \begin{center}
		\scalebox{.8}{\input{svflpf.pgf}}
    \end{center}
    \caption{Analog vs Digital LPF Comparison}
\end{figure}
